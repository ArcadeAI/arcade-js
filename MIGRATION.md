...
This guide outlines the changes and steps needed to migrate your codebase to the latest version of the Arcade TypeScript SDK.
...
To use it, upgrade the `@arcadeai/arcadejs` package, then run `./node_modules/.bin/arcadeai-arcadejs migrate ./your/src/folders` to update your code.
...
- `client.workers.list()`
- `client.workers.tools()`

### Removed `httpAgent` in favor of `fetchOptions`

The `httpAgent` client option has been removed in favor of a [platform-specific `fetchOptions` property](https://github.com/stainless-sdks/arcade-engine-typescript#fetch-options).
This change was made as `httpAgent` relied on `node:http` agents which are not supported by any runtime's builtin fetch implementation.

If you were using `httpAgent` for proxy support, check out the [new proxy documentation](https://github.com/stainless-sdks/arcade-engine-typescript#configuring-proxies).
...
#### Refactor of `@arcadeai/arcadejs/core`, `error`, `pagination`, `resource` and `uploads`

Much of the `@arcadeai/arcadejs/core` file was intended to be internal-only but it was publicly accessible, as such it has been refactored and split up into internal and public files, with public-facing code moved to a new `core` folder and internal code moving to the private `internal` folder.
...
```typescript
// Before
import '@arcadeai/arcadejs/error';
import '@arcadeai/arcadejs/pagination';
import '@arcadeai/arcadejs/resource';
import '@arcadeai/arcadejs/uploads';

// After
import '@arcadeai/arcadejs/core/error';
import '@arcadeai/arcadejs/core/pagination';
import '@arcadeai/arcadejs/core/resource';
import '@arcadeai/arcadejs/core/uploads';
```

If you were relying on anything that was only exported from `@arcadeai/arcadejs/core` and is also not accessible anywhere else, please open an issue and we'll consider adding it to the public API.

#### Resource classes

Previously under certain circumstances it was possible to import resource classes like `Admin` directly from the root of the package. This was never valid at the type level and only worked in CommonJS files.
Now you must always either reference them as static class properties or import them directly from the files in which they are defined.

```typescript
// Before
const { Admin } = require('@arcadeai/arcadejs');

// After
const { Arcade } = require('@arcadeai/arcadejs');
Arcade.Admin; // or import directly from @arcadeai/arcadejs/resources/admin/admin
```

#### Cleaned up `uploads` exports

As part of the `core` refactor, `@arcadeai/arcadejs/uploads` was moved to `@arcadeai/arcadejs/core/uploads`
...
```typescript
import { type Uploadable, toFile } from '@arcadeai/arcadejs/core/uploads';
```
...
```typescript
// Before
import { APIClient } from '@arcadeai/arcadejs/core';
...
```ts
// Before
Arcade.fileFromPath('path/to/file');
...
The `@arcadeai/arcadejs/shims` imports have been removed. Your global types must now be [correctly configured](#minimum-types-requirements).
...
### `@arcadeai/arcadejs/src` directory removed

Previously IDEs may have auto-completed imports from the `@arcadeai/arcadejs/src` directory, however this
directory was only included for an improved go-to-definition experience and should not have been used at runtime.

If you have any `@arcadeai/arcadejs/src/*` imports, you will need to replace them with `@arcadeai/arcadejs/*`.

```ts
// Before
import Arcade from '@arcadeai/arcadejs/src';
