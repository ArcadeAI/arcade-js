...
export const handler = async (client: Arcade, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.admin.authProviders.delete(id)));
  } catch (error) {
